...
for DIR in $(ls $GDC); do
  ...
  if [[ $NAME =~ "TCGA" ]]; then
    ##echo out shell scripts for R
    mkdir -p $BASEDIR/results/$NAME
    echo -e "Rscript --vanilla scripts/GRanges_geneLists_test.call.R \
      $RLIBPATH \
      $BASEDIR \
      data/$NAME \
      results/$NAME \
      data/geneLists \
      run \n;
    Rscript --vanilla scripts/protein_domain_mutations.call.R \
      $BASEDIR \
      results/$NAME/${NAME}.somatic.tumour.vcf.glVcfGrList.RData \
      data/geneLists\n" >> $BASEDIR/results/$NAME/$NAME".R"
    if [[ -d data/$NAME ]]; then
    ...
    if [[ $DRYRUN != "" ]]; then
      #R run
      Rscript --vanilla scripts/GRanges_geneLists_test.call.R \
        $RLIBPATH \
        $BASEDIR \
        data/$NAME \
        results/$NAME \
